...
| `MSHL` | no       | One level of a mesh's cluster graph: a cluster count, an index total, then per cluster a triangle span, a bounding sphere, a normal cone and two errors, then the level's indices. **One chunk per level**, because a level is a complete alternative mesh and folding them together makes one payload whose parts are only meaningful as a set — where separate chunks let a loader take the coarsest first and refine, which is the argument `coarseFirst.ts` already makes for splats. `parentError` is infinite on the coarsest level, which is how it says it has no parent: a float carries that exactly, so no sentinel and no special case. **Additive**, so a reader that does not know the code skips it by its length and loses only the GPU-driven path — which it could not have drawn anyway.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `DTEX` | no       | One material as a **decode program over a latent**, rather than as pictures: **which `MATL` entry it decodes**, a latent grid and its components, the channel specs, the decode graph's nodes, a small network's shape and weights, and a tile table carrying an offset, a length and a **content hash** per tile so two identical tiles can share one payload. **From 1.17 that table is the streaming unit and can be one**: the header carries `tileSize`, the payload is **tile-major** where there are tiles — each tile's texels together, in the grid's order across a row and then down — and the hash is `hashTile`'s **64 bits**, the number residency addresses a tile by. It was a flat list of offsets into a row-major image with a 32-bit hash nothing wrote, and a tile of a row-major image is not contiguous, so no offset and length could name one. `tileCount` of 0 means the payload is one image, which is what every material small enough not to stream still is. **The chunk carries no semantics**, because `@driftengine/drft` has no dependencies and does not know what an operation means — the types are plain arrays that `@driftengine/texture`'s own types satisfy structurally, the arrangement `Terrain` already has with `Heightfield` across the physics boundary. **It validates containment, not meaning**: a tile whose bytes run past the payload and a result register no node writes are refused, because those hand a reader arbitrary memory or an uninitialised value; whether the program is one an interpreter can run is `validateDecodeGraph`'s question. **No per-chunk version**, matching every other chunk here — the file's version covers the format and a second one is a second thing to keep in step. **Additive**, so a reader that does not know the code skips it by its length and loses only the material, which it had no way to decode. **The material index is inside the chunk rather than in the order the chunks appear**, because a file may carry a program for some of its materials and not others — a scene where one surface came from a capture and the rest were authored — and a reader pairing by position would hand the wrong material the wrong texture with nothing to fail on. |
| `SDFV` | no       | One signed distance field per mesh that has one, so indirect light can be traced through the geometry rather than against it: a count, a fixed-stride entry table carrying a mesh ordinal, three dimensions and six bounds, then every entry's samples back to back as `f32`. **The table is separate from the samples** so a reader can find one mesh's field without parsing every field before it, and so every sample block lands on a four-byte boundary and can be viewed rather than copied — `MSHL` takes its views the same way. **`f32` rather than anything smaller**, because a quantised distance is not a safe step and a march that trusts a value rounded up walks through the surface it was meant to stop at. **One chunk for the file with the ordinal per entry**, which is `SUBS`'s rule and `MORP`'s scar. **No per-chunk version**, matching every other chunk here. **Additive**, so a reader that does not know the code skips it by its length and loses only indirect light, which it had no way to trace anyway. Added at 1.13.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `NNET` | no       | The weights of the small networks the engine evaluates, each found by a role — a FourCC the consumer chooses, so a file can carry several and a reader asks for the one it wants. A count, a fixed-stride entry table carrying the role, the input and output widths, up to eight hidden widths, the precision and the weight count, then every network's weights back to back in `evalNetwork`'s layout: each layer's matrix row by row, then its biases. **Sixteen or thirty-two bits per network**, half-precision stored as the bits a device uploads; each block is padded to a four-byte boundary so every one can be viewed rather than copied. **The weight count must be the shape's**, because a network with every weight present and a shape that lays them out differently reads as a working file and evaluates wrongly everywhere. **Two networks with one role are refused.** **No per-chunk version**, and **additive**: a reader that does not know the code skips it by its length and loses only the networks. Added at 1.14.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `NGRF` | no       | Networks built from operators rather than dense layers — the form a transformer takes, which `NNET`'s perceptron table cannot describe — each found by a role. A count, then per graph its role, the byte lengths of its structure and its payload, the structure as UTF-8 JSON (inputs, outputs, nodes naming an operator, its input and output values and its attributes, and a tensor directory of name, shape, precision and payload offset) padded to four bytes, then the tensors, each aligned to four bytes so it can be viewed rather than copied. **Sixteen or thirty-two bits per tensor.** **The chunk carries no semantics**: a reader checks the structure field by field and every tensor's bytes against the payload before viewing one, and `@driftengine/texture` validates the graph, naming any operator it lacks. **Two graphs with one role are refused.** **No per-chunk version**, and **additive**. Added at 1.15, when a file holding only networks became an asset: a converted model is weights and nothing else.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `NAVM` | no       | A polygon mesh a character can walk, **with the placement it was built at**: vertex and polygon counts, the slots each polygon has, the origin and cell size, then the vertices as x and z in cell units, the polygons, the polygon across each edge and the region each came from. The placement is inside the chunk because the vertices are _cell indices_ — a mesh that arrives without the origin and the cell size is in the wrong units at the wrong place, and looks loaded. **Containment is checked and walkability is not**: an index past the end of a table is refused on both sides, because it walks a character off the mesh; whether the polygons describe a surface anybody can cross is `@driftengine/nav`'s question. **No per-chunk version**, and **additive**: a reader that does not know the code skips it by its length and loses only the navigation. Added at 1.16.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `ENTS` | no       | The things in a scene, as the entity model wrote them: a byte length, then the scene as UTF-8 JSON, padded to four bytes. **Text in a binary container, on purpose.** Every other chunk here holds numbers whose meaning this format knows; a scene holds a _consumer's_ component fields, whose types are declared in that consumer's own code. Encoding them would mean encoding somebody else's type system, extended every time they add a field, and a field the writer did not know would silently not be there. The chunk refuses text that is not a scene — a version, schemas and a list of entities with components — and leaves what a component _means_ to `deserializeWorld`, which answers in words. **No per-chunk version**: the scene carries its own. **Additive**. Added at 1.16.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 1.12    | `COLL` defined, `DTEX`                                                                                                                                       | Additive, and **recorded here late**: both shipped at 1.12 while this table stopped at 1.11, so the row exists because 1.13 went looking for where to put itself. `COLL`'s FourCC was claimed in v1 and carried no bytes until 1.12, which is the history `NODE` and `ANIM` also had.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 1.13    | `SDFV`                                                                                                                                                       | Additive. An object's signed distance field, baked offline by `bakeObjectSdf` and composed around the camera at runtime — neither end can do the other's job and the container is what joins them. A 1.12 reader skips the code by its length and opens the file as the geometry it also holds.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 1.14    | `NNET`                                                                                                                                                       | Additive. Network weights by role, in single or half precision — the chunk Wave 2B planned and Wave 3A's refinement tier consumes. A 1.13 reader skips the code by its length and opens the file as everything else it holds.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 1.15    | `NGRF`, and a file of networks alone is an asset                                                                                                             |
| 1.16    | `NAVM` and `ENTS`                                                                                                                                            | Additive. A way across a scene and the things in it — what a capture proposes and an editor saves. A 1.15 reader skips both codes by their length and opens the file as the geometry it always was.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 1.17    | `DTEX` tiles that can be streamed                                                                                                                            | **A break inside `DTEX` and nowhere else.** The chunk gains `tileSize`, its table is the grid in the grid's order, its hash is 64 bits, and a tiled payload is tile-major. Nothing had ever written a tile table, so no file carries one — which is why this is a change rather than a second table beside the first.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
